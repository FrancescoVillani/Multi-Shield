"""
Modified AutoAttack for Multi-Shield

This file is a modified version of the AutoAttack implementation from RobustBench.
Original source: https://github.com/fra31/auto-attack

Modifications for Multi-Shield:
- Added 'apgd-ce-rejection-ms' and 'apgd-dlr-rejection-ms' attack variants that
  target the Multi-Shield rejection mechanism by optimizing a combined loss that
  considers both the misclassification and avoiding the rejection class.
- Modified accuracy computation to account for the rejection class in evaluations.
- Integrated with modified_autopgd_base.py for the underlying APGD implementation.
...
class AutoAttack():
    def __init__(self, model, rejection_class_index, norm='Linf', eps=.3, seed=None, verbose=True,
        ...
        if not self.is_tf_model:
            from attacks.modified_autopgd_base import APGDAttack
            self.apgd = APGDAttack(self.model, rejection_class_index=rejection_class_index, n_restarts=5, n_iter=100, verbose=False,
            ...
            from attacks.modified_autopgd_base import APGDAttack_targeted
        ...
        with torch.no_grad():
            ...
            if state.robust_flags is None:
                ...
                for batch_idx in range(n_batches):
                    ...
                    correct_batch = (y.eq(output)) | (output.eq(self.apgd.rejection_class_index)) if self.apgd.rejection_class_index is not None else (y.eq(output))
            ...
            for attack in attacks_to_run:
                ...
                for batch_idx in range(n_batches):
                    ...
                    if attack == 'apgd-ce':
                        ...
                        adv_curr = self.fab.perturb(x, y)
                    # MODIFIED
                    elif attack == 'apgd-ce-rejection-ms':
                        # apgd on dlr loss
                        self.apgd.loss = 'ce-rejection-ms'
                        self.apgd.seed = self.get_seed()
                        adv_curr = self.apgd.perturb(x, y)  # cheap=True
                    elif attack == 'apgd-dlr-rejection-ms':
                        # apgd on dlr loss
                        self.apgd.loss = 'dlr-rejection-ms'
                        self.apgd.seed = self.get_seed()
                        adv_curr = self.apgd.perturb(x, y)  # cheap=True
                    elif attack == 'apgd-ce-rejection-nr':
                        # apgd on dlr loss
                        self.apgd.loss = 'ce-rejection-nr'
                        self.apgd.seed = self.get_seed()
                        adv_curr = self.apgd.perturb(x, y)  # cheap=True
                    elif attack == 'apgd-dlr-rejection-nr':
                        # apgd on dlr loss
                        self.apgd.loss = 'dlr-rejection-nr'
                        self.apgd.seed = self.get_seed()
                        adv_curr = self.apgd.perturb(x, y)  # cheap=True
                    else:
                    ...
                    # MODIFIED
                    false_batch = (~y.eq(output).to(robust_flags.device)) & (~output.eq(10).to(robust_flags.device))
