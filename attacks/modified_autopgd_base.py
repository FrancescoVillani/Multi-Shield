...
#
"""
Modified Auto-PGD for Multi-Shield

This file is a modified version of the APGD implementation from AutoAttack/RobustBench.
Original source: https://github.com/fra31/auto-attack

Modifications for Multi-Shield:
- Added 'ce-rejection-ms' and 'dlr-rejection-ms' loss functions that combine the
  standard loss with a term pushing away from the rejection class.
- Added 'ce-rejection-nr' and 'dlr-rejection-nr' for baseline comparison with
  Neural Rejection methods.
- Modified prediction checks to account for the rejection class index.
- Added rejection_class_index parameter to APGDAttack class.

Original AutoAttack paper: https://arxiv.org/abs/2003.01690
"""
...
class APGDAttack():
    ...
    def __init__(
        ...
        self.y_target = None
        self.logger = logger
        self.rejection_class_index = rejection_class_index
    ...
    def attack_single_run(self, x, y, x_init=None):
        ...
        # MODIFIED
        def rejection_ce_ms(logits, y):
            criterion_indiv = nn.CrossEntropyLoss(reduction='none')
            loss1 = criterion_indiv(logits, y)
            if self.rejection_class_index is not None:
                loss2 = criterion_indiv(logits, torch.full_like(y, self.rejection_class_index))
                return loss1 + loss2
            return loss1
        
        def rejection_dlr_ms(logits, y):
        ...
        # MODIFIED
        def rejection_ce_nr(logits, y):
            criterion_indiv = nn.CrossEntropyLoss(reduction='none')
            logits = logits[:, :-1]  # Ignore the logits of the last class
            loss1 = criterion_indiv(logits, y)
            return loss1
        
        def rejection_dlr_nr(logits, y):
            criterion_indiv = self.dlr_loss
            logits = logits[:, :-1]  # Ignore the logits of the last class
            loss1 = criterion_indiv(logits, y)
            return loss1
        ...
        if not self.is_tf_model:
            ...
            # MODIFIED
            elif self.loss == 'ce-rejection-ms':
                criterion_indiv = rejection_ce_ms
            elif self.loss == 'dlr-rejection-ms':
                criterion_indiv = rejection_dlr_ms
            elif self.loss == 'ce-rejection-nr':
                criterion_indiv = rejection_ce_nr
            elif self.loss == 'dlr-rejection-nr':
                criterion_indiv = rejection_dlr_nr
            else:
        ...
        for _ in range(self.eot_iter):
            if not self.is_tf_model:
                with torch.enable_grad():
                    ...
                    loss = loss_indiv.sum()
                    #print("loss", loss)
                #print( torch.autograd.grad(loss, [x_adv], allow_unused=True))
                grad += torch.autograd.grad(loss.to(self.device), [x_adv.to(self.device)])[0].detach()
            else:
        ...
        for i in range(self.n_iter):
            ...
            # MODIFIED
            if self.rejection_class_index is not None:
                pred = (logits.detach().max(1)[1] == y) | (logits.detach().max(1)[1] == self.rejection_class_index)
            else:
    ...
    def perturb(self, x, y=None, best_loss=False, x_init=None):
        ...
        # MODIFIED
        assert self.loss in ['ce', 'dlr', 'ce-rejection-ms', 'dlr-rejection-ms', 'ce-rejection-nr', 'dlr-rejection-nr']  # 'ce-targeted-cfts'
        ...
        if self.loss != 'ce-targeted':
            # MODIFIED
            if self.rejection_class_index is not None:
                acc = (y_pred == y) | (y_pred==self.rejection_class_index)
            else:
                acc = (y_pred == y) 
        else:
            acc = y_pred != y
